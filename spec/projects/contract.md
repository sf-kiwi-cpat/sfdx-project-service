...
- A single template may declare **both** fields — they coexist and are surfaced independently. Neither field's presence implies or affects the other. The asymmetric cases are part of the contract and are exercised directly: a template shipping **only** `seedMessages` surfaces `seedMessages` with `initialMessages` omitted, and a template shipping **only** `initialMessages` surfaces `initialMessages` with `seedMessages` omitted
...
- **initialMessages / seedMessages independence (contract)**: spec-owned fixture templates prove the asymmetric cases — a template shipping only `seedMessages` surfaces it with `initialMessages` absent, and a template shipping only `initialMessages` surfaces it with `seedMessages` absent (both on create and GET).
