...
- The repo's `/.npmrc` pins the registry to public npm so the root lockfile stays portable across CI runners and dev machines outside the SF corp network. When regenerating `package-lock.json`, ensure `.npmrc` is present in the working tree before `npm install` — otherwise npm walks up to the user-level `~/.npmrc` (Nexus) and silently re-Nexuses the lockfile. Procedure: `rm -rf node_modules package-lock.json && npm install`. The `rm -rf package-lock.json` is load-bearing — npm honors existing `resolved` URLs and won't refetch otherwise. CI fails fast on a Nexus-tagged root lockfile because the rewrite step (`.github/workflows/ci.yml`) is narrowed to `./templates` and no longer covers root. Template lockfiles still go through Nexus pending #283.
